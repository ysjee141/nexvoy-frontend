// src/utils/currency.ts에서 이관 — 무변형 (순수 함수)
...
export function getCurrencyFromTimezone(timezone: string): CurrencyInfo {
  ...
  if (TIMEZONE_CURRENCY_MAP[timezone]) return TIMEZONE_CURRENCY_MAP[timezone]
  const prefix = timezone.split('/')[0]
  if (prefix === 'Europe') return { code: 'EUR', symbol: '€', locale: 'fr-FR', name: '유로' }
  if (prefix === 'America') return DEFAULT_CURRENCY
  ...
  try {
    return new Intl.NumberFormat(currency.locale, {
      ...
      maximumFractionDigits:
        currency.code === 'JPY' || currency.code === 'KRW' || currency.code === 'VND' ? 0 : 2,
    }).format(amount)
  } catch {
