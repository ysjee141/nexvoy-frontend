...
2. `cd apps/mobile`
3. `pnpm exec expo install react-native-webrtc @config-plugins/react-native-webrtc expo-dev-client`
4. `apps/mobile/app.json` plugins에 `@config-plugins/react-native-webrtc` 추가
5. `pnpm exec eas build --profile development --platform ios`
6. `pnpm exec eas build --profile development --platform android`
7. 실기기에서 peer connection/data channel 생성 확인
8. foreground/background 전환 후 연결 close/reconnect 로그 확인
9. WebRTC disabled/unavailable 상태에서 Supabase backup restore flow가 유지되는지 확인
