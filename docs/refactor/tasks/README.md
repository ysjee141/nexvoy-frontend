...
- [ ] `TASK-027-mobile-yjs-runtime-adapter.md`: Mobile Yjs runtime adapter로 Web/Mobile 공통 update format 적용
...
`TASK-021`은 Web-to-Web P2P 연결을 최초로 증명했고, `TASK-022`는 일반 계정 trip의 document registry 등록 갭을 해소했다. `TASK-023`은 Mobile 배선까지 확장했고, `TASK-024`는 Web-to-Web Yjs update 교환을 구현했다. Mobile P2P 데이터 적용은 별도 format을 만들지 않고 Yjs update를 canonical format으로 유지하는 `TASK-027`이 권장 경로다. 다음 권장 순서는
`TASK-027`(Mobile Yjs runtime adapter) → `TASK-025`(UI) → `TASK-026`(생명주기 하드닝)이다. 통합
