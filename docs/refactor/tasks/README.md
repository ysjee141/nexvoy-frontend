...
- [ ] `TASK-021-web-signaling-channel-and-data-channel-handshake.md`: Web 시그널링 채널(Supabase Realtime Broadcast) + 데이터 채널 배선으로 P2P fast path 최초 연결 증명 (예정)
...
`TASK-021`은 `ADR-004`가 유보했던 시그널링 전송 계층을 `ADR-012`(Supabase Realtime Broadcast)로 결정하고, Web에서 시그널링 채널과 데이터 채널을 실제로 연결해 P2P fast path가 최초로 동작함을 증명하는 계획 문서다(구현 미착수). Mobile 시그널링 배선과 Yjs update 실제 교환은 `TASK-021` 이후 별도 task로 이관한다(예: Mobile signaling wiring, Web-Mobile 교차 연결, updates replay 지원).
