...
- **구현 내용**:
    - `ProfilePage` 하단 '회원 탈퇴' 위에 '이용약관 및 개인정보 처리방침' 메뉴 추가 완료.
...
- **구현 내용**:
    - `POST_NOTIFICATIONS` 권한 선언 완료 (Android 13+ 알림 허용).
    - `SCHEDULE_EXACT_ALARM` 권한 선언 완료 (일정 알람 정확도 향상).
...
- **구현 내용**:
    - `server.cleartext` 설정을 `false`로 변경 완료 (운영 환경 대비).
    - `androidScheme`을 `https`로 설정 완료.
