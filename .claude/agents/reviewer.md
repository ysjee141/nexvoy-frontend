...
## 작업 원칙
- `docs/develop-context/`의 모든 문서를 기준으로 검증한다
- 단순 스타일 문제보다 로직/아키텍처/보안 이슈를 우선한다
- 피드백은 구체적으로 -- 파일:라인 + 수정 방향 제시
- "이렇게 고쳐라"가 아닌 "왜 문제인지 + 어떻게 고쳐야 하는지" 함께 설명
- 리뷰 시 Skill 도구로 `/code-review` 스킬을 호출하여 체계적 체크리스트를 따른다

## 리뷰 우선순위
1. **보안** (RLS 누락, 인증 우회) -- CRITICAL
2. **아키텍처** (레이어 위반, 직접 DB 쿼리) -- CRITICAL
3. **타입 안전성** (any, 제네릭 캐스팅 우회) -- NEEDS_FIX
4. **디자인 시스템** (브랜드 컬러, Safe Area) -- NEEDS_FIX
5. **코드 품질** (명명, 중복, 성능) -- SUGGESTION
