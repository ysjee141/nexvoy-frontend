                ...
                {loading ? (
                    ...
                    <div className={css({ display: 'flex', flexDirection: 'column', gap: '12px', overflowY: 'auto', flex: 1, pr: '4px' })}>
                        {templates.map(template => (
                            ...
                            >
                                <span className={css({ fontWeight: '600', fontSize: '14px', color: '#333', flex: 1 })}>{template.title}</span>
                                <button
                                    onClick={() => handleApplyTemplate(template.id)}
                                    disabled={loadingTemplateId === template.id}
                                    className={css({ display: 'flex', alignItems: 'center', gap: '6px', px: '12px', py: '8px', bg: '#4285F4', color: 'white', borderRadius: '8px', border: 'none', cursor: 'pointer', fontSize: '12px', fontWeight: 'bold', whiteSpace: 'nowrap', _hover: { bg: '#3367d6' }, _active: { transform: 'scale(0.95)' }, _disabled: { opacity: 0.5, cursor: 'not-allowed' } })}
                                >
