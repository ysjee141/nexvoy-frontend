...
export default function NewTripModal({ isOpen, onClose, onSuccess }: NewTripModalProps) {
    ...
    const isDirty = useCallback(() => {
        return destination !== '' || startDate !== '' || endDate !== '' || adults !== 1 || childrenCount !== 0
    }, [destination, startDate, endDate, adults, childrenCount])
    ...
    useModalBackButton(isOpen, handleClose, 'newTripModal')
                ...
                <div className={css({ p: { base: '20px', sm: '32px' }, flex: 1 })}>
                    <form onSubmit={handleSubmit} className={css({ display: 'flex', flexDirection: 'column', gap: '28px' })}>
                        ...
                        <div className={css({ textAlign: 'center', mb: '4px' })}>
                            ...
                            <h3 className={css({ fontSize: '22px', fontWeight: '800', color: 'brand.ink', mb: '6px', letterSpacing: '-0.03em' })}>어디로 떠나시나요?</h3>
                            <p className={css({ color: 'brand.muted', fontSize: '15px', fontWeight: '500' })}>새로운 여행의 시작을 함께 계획해봐요.</p>
                        </div>
                                ...
                                <div className={css({
                                    ...
                                    <button
                                        ...
                                        className={css({
                                            ...
                                            bg: adults <= 1 ? 'rgba(0,0,0,0.05)' : 'brand.primary/10', color: adults <= 1 ? '#A0AEC0' : 'brand.primary',
                            ...
                            <div>
                                ...
                                <div className={css({
                                    ...
                                    <button
                                        ...
                                        className={css({
                                            w: '36px', h: '36px', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center',
                                            bg: childrenCount <= 0 ? 'rgba(0,0,0,0.05)' : 'brand.primary/10', color: childrenCount <= 0 ? '#A0AEC0' : 'brand.primary',
                        ...
                        <button
                            ...
                            disabled={loading}
                            className={css({
                                w: '100%', py: '16px', bg: 'brand.primary', color: 'white', borderRadius: '8px', fontWeight: '800',
                                fontSize: '16px', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center',
                                justifyContent: 'center', gap: '10px', boxShadow: 'none',
                                transition: 'all 0.3s', _disabled: { opacity: 0.6 },
                                _hover: { bg: 'brand.primaryActive', boxShadow: 'airbnbHover' }, 
                                _active: { transform: 'scale(0.97)' }
                            })}
                        >
                            {loading ? <><Loader2 size={20} className={css({ animation: 'spin 1.5s linear infinite' })} /> 저장 중...</> : <><Check size={20} strokeWidth={3} /> 여행 계획 시작하기</>}
                        </button>
                    </form>
                </div>
            </div>
