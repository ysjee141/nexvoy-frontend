        ...
        <div className={css({ 
            ...
            <header className={css({ 
                ...
                <div className={css({ display: 'flex', alignItems: 'center', px: '16px', w: '100%', maxW: '720px', mx: 'auto' })}>
                    ...
                    <h1 className={css({ fontSize: '18px', fontWeight: 'bold', color: '#111' })}>회원 탈퇴</h1>
                </div>
            </header>
            ...
            <main className={css({ maxW: '720px', mx: 'auto', p: '24px 20px', pb: 'calc(24px + env(safe-area-inset-bottom))' })}>
                {step === 1 ? (
                    <div className={css({ bg: 'white', borderRadius: '24px', p: { base: '32px 24px', sm: '48px' }, textAlign: 'center', boxShadow: '0 8px 24px rgba(0,0,0,0.04)' })}>
                        ...
                        <h2 className={css({ fontSize: '24px', fontWeight: '800', color: '#111', mb: '12px' })}>
                            정말 Onvoy를 떠나시겠어요?
                        </h2>
                        
                        {(stats.totalTrips > 0 || stats.totalItems > 0) ? (
                            <p className={css({ fontSize: '15px', color: '#555', lineHeight: 1.6, mb: '32px', wordBreak: 'keep-all' })}>
                                지금 떠나시면 그동안 함께 했던 <strong className={css({ color: '#4285F4' })}>{stats.totalTrips}개의 소중한 여행 일정</strong>과 <strong className={css({ color: '#34A853' })}>{stats.totalItems}개의 짐 챙기기 기록</strong>이 모두 삭제되며, 영구적으로 복구할 수 없습니다. 계속 함께해 주시면 안 될까요?
                            </p>
                        ) : (
                            <p className={css({ fontSize: '15px', color: '#555', lineHeight: 1.6, mb: '32px', wordBreak: 'keep-all' })}>
                    ...
                    <div className={css({ bg: 'white', borderRadius: '24px', p: { base: '32px 24px', sm: '48px' }, boxShadow: '0 8px 24px rgba(0,0,0,0.04)' })}>
                        ...
                        <h2 className={css({ fontSize: '20px', fontWeight: 'bold', color: '#111', mb: '8px' })}>마지막 확인</h2>
                        ...
                        <input
                            ...
                            placeholder="탈퇴"
                            className={css({ w: '100%', p: '14px 16px', border: '1px solid #ddd', borderRadius: '12px', fontSize: '16px', outline: 'none', mb: '24px', transition: 'border-color 0.2s', _focus: { borderColor: '#111' } })}
                        />
