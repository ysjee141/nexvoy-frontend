...
export default function ChecklistPage(props: {
    ...
    const toggleItem = async (itemId: string, currentStatus: boolean) => {
        ...
        setItems(prev => prev.map(item => item.id === itemId ? { ...item, is_checked: !currentStatus } : item))
        ...
        <div className={css({ bg: 'white', p: '24px', borderRadius: '16px', boxShadow: '0 4px 12px rgba(0,0,0,0.03)' })}>
            ...
            {totalItems === 0 && !isAdding ? (
                ...
                <div className={css({ display: 'flex', flexDirection: 'column', gap: '24px' })}>
                    {groupBy === 'category' ? (
                        CATEGORIES.map(cat => {
                            const categoryItems = items.filter(item => item.category === cat || (!item.category && cat === '기타'))
