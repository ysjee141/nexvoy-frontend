...
import type { TripProductState } from '@nexvoy/core/product/models'
...
export function createWebAuthorityChecklistRepository(
  ...
  return {
    ...
    createItem: async (checklistId, input) => {
      ...
      if (!tripId) throw new Error('Checklist was not found.')
      const role = await resolveCurrentTripRole(supabase, repositories, tripId)
      const writeRepositories = await createWebAuthorityDocumentRepositories(supabase, { actorRole: role })
      ...
      return toMutationResult(result.state, itemId)
    },
    updateItem: async (itemId, input) => {
      const repositories = await createWebAuthorityDocumentRepositories(supabase)
      const resolved = await resolveTripAndItem(repositories, itemId)
      if (!resolved) throw new Error('Checklist item was not found.')
    ...
    toggleItemForUser: async (input) => {
      ...
      const checklists = await repositories.checklists.getChecklist(result.state.trip.id)
...
function toChecklistMemberRow(member: TripMemberReadModel): ChecklistMemberSnapshot {
  return {
    ...
    created_at: '',
    email: member.email,
    profiles: { nickname: member.nickname, email: member.email },
  }
}
...
async function resolveTripAndItem(
  ...
  for (const trip of trips) {
    const checklists = await repositories.checklists.getChecklist(trip.id)
    const item = checklists.flatMap((checklist) => checklist.items)
      .find((candidate) => candidate.id === itemId)
    if (item) return { tripId: trip.id, item }
  }
  return null
}
...
function toMutationResult(
  ...
  const item = state.checklistItems[itemId]
  ...
  return {
    ...
    assignees: Object.values(state.checklistItemAssignees)
